...
router.post('/excel_users', uploadExcel.single('file'), async function (req, res, next) {
    if (!req.file) {
        return res.send({ message: "file khong duoc rong" })
    }
    ...
    let userRole = await roleModel.findOne({ name: 'user' }) // or 'USER'/'User' depending on DB
    if (!userRole) {
        // Fallback: create the role if it doesn't exist just in case
    ...
    for (let index = 2; index <= worksheet.rowCount; index++) {
        ...
        if (!username || typeof username === 'object') {
            username = username?.text || username?.richText?.[0]?.text;
        }
        if (!email || typeof email === 'object') {
            email = email?.text || email?.richText?.[0]?.text;
        }

        if (!username) errorsRow.push("username is empty or invalid");
        if (!email) errorsRow.push("email is empty or invalid");

        if (errorsRow.length > 0) {
            result.push({ success: false, data: errorsRow })
            continue;
        }
        ...
        if (existingUser) {
            result.push({ success: false, data: ["Username or Email already exists"] })
            continue;
        }

        let password = crypto.randomBytes(8).toString('hex'); // 16 characters
        ...
        try {
            ...
            // The pre('save') hook in schemas/users.js will hash the password
            ...
            result.push({ success: true, data: { username, email, message: "User created & email sent" } })
        } catch (error) {
            await session.abortTransaction()
            await session.endSession()
